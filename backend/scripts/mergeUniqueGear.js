...
const oldGear = require('../src/data/uniqueGears');

// HOW MANY LEVELS TO CREATE
const MAX_LEVELS = 1;
// ADDITIONAL UNIQ GEAR LEVEL OFFSET
const LEVEL_OFFSET = 1111000;
...
for (let lvl = 0; lvl <= MAX_LEVELS; lvl += 1) {
  ...
  const newLevelGear = Object.fromEntries(
    newGear
      .filter(v => v.id >= lvl * 1000 + LEVEL_OFFSET && v.id < (lvl + 1) * 1000 + LEVEL_OFFSET)
      .map(v => [v.id, v]),
  );
