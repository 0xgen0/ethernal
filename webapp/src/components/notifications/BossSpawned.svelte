<script>
  ...
  const indefArticle = /^[aeiou]/i.test(currentMonster.name) ? 'An' : 'A';
</script>
...
<p>
  ...
  <img class="icon" alt="{currentMonster.name}" src="{monsterImage(currentMonster.image)}" />
  {indefArticle}
  <em>{currentMonster.name}</em>
